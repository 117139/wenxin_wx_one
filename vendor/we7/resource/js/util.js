...
util.url = function (action, querystring) {
    var app = getApp();
    var url = app.siteInfo.siteroot + '?t=' + app.siteInfo.uniacid + '&from=wxapp&';
  ...
  if (flag == 1) {
    ...
    wx.login({
      ...
      //  app.siteInfo.siteroot + '?i=' + app.siteInfo.uniacid + '&t=' + app.siteInfo.multiid + '&v=' + app.siteInfo.version + '&from=wxapp&';
        var url_openid = appcofing.siteInfo.siteroot + '?c=auth&a=session&do=openid&t=' + appcofing.siteInfo.acid + '&key=' + appcofing.siteInfo.key + '&secret=' + appcofing.siteInfo.secret;
        util.request({
          url: url_openid,
          data: { code: res.code },
          cachetime: 0,
            ...
            if (session.data.errno == 0) { 
              ...
              wx.getUserInfo({
                success: function (wxInfo) {
                  ...
                  wx.setStorageSync('userInfo', userInfo);
                  var url_userinfo = appcofing.siteInfo.siteroot + '?c=auth&a=session&do=userinfo&t=' + appcofing.siteInfo.acid + '&key=' + appcofing.siteInfo.key + '&secret=' + appcofing.siteInfo.secret;
                  util.request({
                    url: url_userinfo,
                    data: {
...
util.getGoodsDetail = function(id, cb) {
  ...
  //var base_url = app_config.siteInfo.siteroot + '?i=' + app_config.siteInfo.uniacid + '&v=' + app_config.siteInfo.version + '&from=wxapp&';

  //var GoodsDetail = app_config.siteInfo.siteroot + '?c=wxapp&a=good&do=detail&id=' + id + '&t=' + app_config.siteInfo.uniacid;
  util.request({
...
// 编辑或提交用户地址
util.formInfo = function (d, c) {
  wx.showLoading()

  util.request({
    url: 'entry/ewei_shopv2/mobile&r=member.info.submitapp',
    method: 'POST',
    header: {
      'content-type': 'application/x-www-form-urlencoded'
    },
